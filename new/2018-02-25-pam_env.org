...
I had a piece of code in my =~/.profile= that checks Racket and Ruby's versions and sets the PATH to point to ~raco~ and ~gem~'s bin directory. Now, =~/.pam_environment= can't do that, so I wrote a [[site:pollen.org][Pollen]] version of the file and rendered / compiled it into the final =~/.pam_environment.=
