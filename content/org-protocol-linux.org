...
2. Change ~Exec=~ from "emacs %F" to "emacsclient -c %U"
   - We use %U to get URLs, instead of just files, to allow =org-protocol://= links. See also: [[https://specifications.freedesktop.org/desktop-entry/latest-single/#exec-variables][the definition of %U and other field codes in the Desktop Entry Specification]].
