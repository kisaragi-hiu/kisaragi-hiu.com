...
After a lot of exploration, I believe the best solution is to define a function partial that replaces [[https://gohugo.io/methods/page/getterms/#article][=.GetTerms=]], providing the same interface while adding support for getting terms from a list page.
...
{{ $term_items := (slice) }}
{{ range (where (partial "func-all-pages" .) "Kind" "term" ) }}
