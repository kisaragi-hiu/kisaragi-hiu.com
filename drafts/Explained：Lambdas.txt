...
`define` has a shorthand version for `(define function-name (lambda (x y) "function body"))`: `(define (function-name x y . rest) "function body")`. This somewhat masks the fact that lambdas are just values.