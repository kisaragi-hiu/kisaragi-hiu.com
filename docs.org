...
Return all pages from all sites of the project.

Compatibility for the old .Site.AllPages, [[https://discourse.gohugo.io/t/deprecations-in-v0-156-0/56732#p-184468-example-a-ndash-range-over-all-pages-in-a-project-4][which is deprecated]] and will be removed in favor of treating each Site in a Project as separate correctly.

We should probably replace use of this with just =.Site.Pages= (all pages of the current site) instead.
...
The utteranc.es script element.

** main-header ()

The primary header content for /projects, /covers, etc.

** global-nav (class)

The “Covers, Illust, Projects…” link list.

Class is the HTML class string on the =nav=.
...
- =nocomment=: Don't show the utterances widget.
