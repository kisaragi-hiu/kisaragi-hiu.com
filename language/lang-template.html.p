◊(define (langcode->headline langcode)
  (cond [(equal? langcode "zh-tw") "中文文章"]
  	[(equal? langcode "en") "English Posts"]
	[else langcode]))

<html>
  <head>
    <title>◊(langcode->headline (select 'title here)) / Kisaragi Hiu</title>
  ...
  <body>
    ◊(->html (get-site-header #:at-index #t #:headline (string-append (langcode->headline (select 'title here)) " / 如月.飛羽")))
